...
wstring passcode;
string video_source = "";
constexpr auto DEFAULT_VIDEO_SOURCE = "";
constexpr auto CONFIG_FILE = "config.json";

//this is set to false by default, you will need to put in your JWT / Auth Signature in your config.json
...
//helper method to check if status is In Meeting
...
bool CanIStartLocalRecording()
{
	...
	if (m_pRecordController)
	{
		...
		if (err != SDKERR_SUCCESS) {
			...
			//ask for permission from Host to start recording
			std::cout << "Requesting...\n";
...
int main()
{
	...
	while (!g_exit && (bRet = GetMessage(&msg, nullptr, 0, 0)) != 0)
	{
		...
		else
		{
			...
			//check if encoder zTscoder.exe is running, this automatically runs when SDK exits, or when meeting is ended by host
	...
	while (!encoderEnded)
	{
		if (encoderHasRan && !IsProcessRunning(processName))
		{
			...
			//save your files somewhere using AWS Cli, such as s3 bucket.
